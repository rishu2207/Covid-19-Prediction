#Stage 1: 180,000 iterations would work.
#Stage 2: 80,000 iterations would work.
using MAT
using Plots
using Measures
using Flux
...
u0 = Float64[209000000.0, 613, 8, 10]
tspan = (0, 74.0)
datasize = 74;
...
P3  =[]
anim = Animation()
datan = Iterators.repeated((), 180000)
opt = ADAM(0.01)
cb = function()
...
Q_Brazil = ((Q_parameter[30] - Q_parameter[1]) /u0[1])

save("QQFinal_QuarHeatmap_Brazil_QSIR_Deadn_Stage2.jld", "Q_Brazil", Q_Brazil, "prediction", prediction, "Q_parameter", Q_parameter, "Reff", Reff, "Transition", Transition, "β_parameter", p3n[52],"γ_parameter", p3n[53],"δ_parameter", p3n[54], "S_NN_all_loss", S_NN_all_loss, "I_NN_all_loss", I_NN_all_loss, "R_NN_all_loss", R_NN_all_loss,"t", t,"Parameters_copy", p3n, "Loss", Loss)

########CLEAN PLOTS###########
D = load("QQFinal_QuarHeatmap_Brazil_QSIR_Deadn_Stage2.jld")
...
savefig("Clean_QQFinal_Quar_Stage2_Brazil_1dn.pdf")
...
Plots.savefig("Clean_QQFinal_Quar_Stage2_Brazil_2d.pdf")

#plot([0, Transition[1]], [80, 80],fill=(0,:lightpink), markeralpha=0, label = "")
plot([Transition[1], datasize], [80, 80],fill=(0,:aliceblue), markeralpha=0, label = "")
scatter!(t,Q_parameter/u0[1], xlims = (0, datasize), ylims = (-0.02,1), xlabel = "Days post 500 infected", ylabel = "Q(t)", label = "Quarantine strength",color = :black, framestyle = :box, grid =:off, legend = :topleft, left_margin = 5mm, bottom_margin = 5mm, foreground_color_legend = nothing, background_color_legend = nothing,  yguidefontsize = 14, xguidefontsize = 14,  xtickfont = font(12, "TimesNewRoman"), ytickfont = font(12, "TimesNewRoman"), legendfontsize = 12)
plot!([2-0.01,2+0.01],[0.0, 1.0],lw=3,color=:green,label="Quarantine imposed in big cities",linestyle = :dash)

Plots.savefig("Clean_QQFinal_Quar_Stage2_Brazil_3dn.pdf")

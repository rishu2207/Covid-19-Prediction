...
############### Spain##########
###########STAGE 1##########
vars = matread("C:/Users/Rishabh/Desktop/2.168/COVID-19-master_latest/COVID-19-master/csse_covid_19_data/csse_covid_19_daily_reports/Rise_Spain_Track.mat")

Random.seed!(50)

Infected = vars["Spain_Infected_All"]
Recovered = vars["Spain_Recovered_All"]
Dead= vars["Spain_Dead_All"]
Time = vars["Spain_Time"]
...
#D = load("CQfinal_QuarHeatmap_Spain_QSIR_Deadn.jld")
...
u0 = Float64[47000000.0, 460, 40, 10]
tspan = (0, 86.0)
datasize = 86;
...
Q_Spain = ((Q_parameter[30] - Q_parameter[1]) /u0[1])



using JLD
save("QQFinal_QuarHeatmap_Spain_QSIR_Deadn_Stage2.jld", "Q_Spain", Q_Spain, "prediction", prediction, "Reff", Reff,"Q_parameter", Q_parameter, "Transition", Transition, "β_parameter", p3n[52],"γ_parameter", p3n[53],"δ_parameter", p3n[54], "S_NN_all_loss", S_NN_all_loss, "I_NN_all_loss", I_NN_all_loss, "R_NN_all_loss", R_NN_all_loss,"t", t, "Parameters", p3,"Parameters_copy", p3n, "Loss", Loss)

###########CLEAN PLOTS###########
D = load("QQFinal_QuarHeatmap_Spain_QSIR_Deadn_Stage2.jld")
...
savefig("Clean_QQFinal_Quar_Stage2_Spain_1dn.pdf")
...
D2 = diff(D1, dims = 1)
Transition = findall(x -> x <0, D2)[1]

plot!([9-0.01,9+0.01],[0.0, 0.6],lw=3,color=:green,label="Government Lockdown imposed",linestyle = :dash)

plot!([Int(Transition[1])-0.01,Int(Transition[1])+0.01],[0.0, 0.6],lw=3,color=:red,label="Inflection point in learnt Q(t)",linestyle = :dash)


Plots.savefig("Clean_QQFinal_Quar_Stage2_Spain_3dn.pdf")

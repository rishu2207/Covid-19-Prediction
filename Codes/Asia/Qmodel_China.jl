#Stage 1: 50,000 iterations would work.
...
u0 = Float64[1393000000.0, 399, 45, 10]
tspan = (0, 130.0)
datasize = 130;
...
save("QQFinal_QuarHeatmap_China_QSIR_Deadn_Stage2.jld", "Q_China", Q_China, "prediction", prediction, "Q_parameter", Q_parameter, "Reff", Reff, "Transition", Transition, "β_parameter", p3n[52],"γ_parameter", p3n[53],"δ_parameter", p3n[54], "S_NN_all_loss", S_NN_all_loss, "I_NN_all_loss", I_NN_all_loss, "R_NN_all_loss", R_NN_all_loss,"t", t, "Parameters", p3,"Parameters_copy", p3n, "Loss", Loss)

########CLEAN PLOTS###########
using MAT
D = load("QQFinal_QuarHeatmap_China_QSIR_Deadn_Stage2.jld")
...
savefig("Clean_QQFinal_Quar_Stage2_China_1dn.pdf")
...
Transitionn = 12
plot!([9-0.01,9+0.01],[0.0, 1.0],lw=3,color=:green,label="Government Lockdown imposed",linestyle = :dash)
...
Plots.savefig("Clean_QQFinal_Quar_Stage2_China_3dnn.pdf")
